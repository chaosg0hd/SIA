...
Class Action {
	...
	function accept_reservation(){
		extract($_POST);
		$accept = $this->db->query("UPDATE reservations set status_id = 1 where id=".$id);
		if($accept)
			return 1;
		else
			console.log("error1");
	...
	function reject_reservation(){
		extract($_POST);
		$reject = $this->db->query("UPDATE reservations set status_id = 3 where id=".$id);
		if($reject)
			return 1;
		else
			console.log("error2");
