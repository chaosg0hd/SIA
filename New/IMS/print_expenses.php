...
<?php
		$expens = $conn->query("SELECT * FROM expenses where id = ".$_GET['id'])->fetch_array();
?>
<div>
<h2 class="text-center">Expenses - <?php echo $expens['id'] ?></h2>
<hr>
</div>
<table>
    <thead>
    ...
    <?php
        ...
        $expenses=$conn->query("SELECT p.*,concat(e.lastname,', ',e.firstname,' ',e.middlename) as ename,e.employee_no,e.salary FROM payroll_items p inner join employee e on e.id = p.employee_id ") or die(mysqli_error());
